    ...
    extends Agent
{
        ...
        extends Object
    {
        ...
        if (this.frontierPitCoordinates.isEmpty()) {
            System.out.println("No frontier squares to explore."); // Debug message
            return null; // No frontier squares to explore.
        }

        Map<Coordinate, Double> probabilities = new HashMap<>();
        double probabilitySum = 0.0;

        for (Coordinate frontier : this.frontierPitCoordinates) {
            double likelihood = this.pitProb; // Start with the base probability of a pit

            // Count the number of adjacent breezes to increase likelihood
            int breezeCount = 0;

            // Check only cardinal directions (N, S, E, W)
            int[][] directions = new int[][]{{0, -1}, {1, 0}, {0, 1}, {-1, 0}};
            for (int[] dir : directions) {
                int adjX = frontier.getXCoordinate() + dir[0];
                int adjY = frontier.getYCoordinate() + dir[1];
                
                Coordinate adjacent = new Coordinate(adjX, adjY);
                Boolean hasBreeze = this.knownBreezeCoordinates.get(adjacent);

                if (hasBreeze != null && hasBreeze) {
                    breezeCount++;
                }
            }

            if (breezeCount > 0) {
                // If there are breezes, the likelihood is the base pit probability raised to the power of breeze count
                likelihood = Math.pow(2, breezeCount);
            } else {
                // If there are no breezes, the likelihood is much lower (safe to explore)
                likelihood *= 0.1; // Arbitrary low value to signify a lower likelihood of pit
            }

            probabilities.put(frontier, likelihood);
            probabilitySum += likelihood;
        }

        // Normalization of probabilities
        if (probabilitySum == 0) {
            System.out.println("Sum of probabilities is zero."); // Debug message
            return null; // No valid probabilities to normalize
        }


        List<Coordinate> leastLikelyPits = new ArrayList<>();
        double minProbability = Collections.min(probabilities.values());

        for (Map.Entry<Coordinate, Double> entry : probabilities.entrySet()) {
            if (Math.abs(entry.getValue() - minProbability) < 0.000001) { // Use a small threshold to compare floating-point numbers
                leastLikelyPits.add(entry.getKey());
            }
        }

        // Choose a random coordinate among the least likely pits
        if (!leastLikelyPits.isEmpty()) {
            int randomIndex = new Random().nextInt(leastLikelyPits.size());
            return leastLikelyPits.get(randomIndex);
        }
        return null;
    }
